...
export class AudioPlaybackManager {
  ...
  private setupAudioEventListeners(): void {
    ...
    this.audioElement.ontimeupdate = () => {
      if (!this.settings.disablePlaybackControlPopover) {
        if (this.isStreamingWithMSE && !this.hasStartedCurrentPlayback) {
          this.updateFloatingPlayerCallback({
            currentTime: this.audioElement.currentTime,
            duration: Infinity,
            isPlaying: false,
    ...
    this.audioElement.onpause = () => {
      if (this.isStreamingWithMSE && !this.hasStartedCurrentPlayback && !this.isSwitchingToFullFile) {
    ...
    this.audioElement.onplay = () => {
      this.hasStartedCurrentPlayback = true;
      this.isPaused = false;
  ...
  private cleanupTemporaryAudioFile(): void {
    this.fileManager.cleanupTempAudioFile().catch(error => console.error('Error cleaning up temporary audio file:', error));
  }
  ...
  private async processTTSStream(cleanText: string, activePlaybackAttemptId: number, outputFormat: string, useMSE: boolean): Promise<void> {
    try {
      if (this.settings.showNotices) {
        new Notice(useMSE ? '正在启动音频流...' : '正在生成语音...');
      }
      this.updateStatusBarCallback(true);
      ...
      readable.on('error', (error: unknown) => {
        ...
        if (this.currentPlaybackId === activePlaybackAttemptId) {
          if (this.settings.showNotices) new Notice('语音生成失败：无法连接 Edge TTS 服务或代理异常。');
          this.stopPlaybackInternal();
        }
      });
      ...
      if (this.currentPlaybackId === activePlaybackAttemptId) {
        if (this.settings.showNotices) new Notice('朗读失败：语音生成流程初始化失败。');
        this.stopPlaybackInternal();
      }
    }
  }
