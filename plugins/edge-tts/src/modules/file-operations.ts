...
export class FileOperationsManager {
  ...
  async cleanupExpiredTempAudioFile(retentionDays: number): Promise<void> {
    if (Platform.isMobile) {
      return;
    }
    ...
    try {
      if (!await adapter.exists(this.tempAudioPath)) return;

      const fileStats = await adapter.stat(this.tempAudioPath);
      if (!fileStats?.mtime) return;

      const safeRetentionDays = Math.max(1, retentionDays || 1);
      const retentionMs = safeRetentionDays * 24 * 60 * 60 * 1000;
      if (Date.now() - fileStats.mtime > retentionMs) {
        await adapter.remove(this.tempAudioPath);
      }
    } catch (error) {
      console.error('Error cleaning up expired temporary audio file:', error);
    }
  }
