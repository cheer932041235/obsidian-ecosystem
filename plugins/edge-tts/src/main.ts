...
import { PlaybackPanelView, VIEW_TYPE_EDGE_TTS_PLAYBACK_PANEL } from './ui/PlaybackPanelView';
...
export default class EdgeTTSPlugin extends Plugin {
	...
	async onload() {
		...
		this.registerView(VIEW_TYPE_EDGE_TTS_PLAYBACK_PANEL, (leaf) => {
			const view = new PlaybackPanelView(leaf, this);
			this.playbackPanelView = view;
			return view;
		});
		...
		// 4. Clean up expired temp audio file from a previous session
		await this.fileManager.cleanupExpiredTempAudioFile(this.settings.tempAudioRetentionDays);
		...
		this.audioManager.setFloatingPlayerCallbacks(
			(data) => {
				this.playbackPanelView?.updatePlaybackState(data);
			},
			() => {
				this.playbackPanelView?.updatePlaybackState({ currentTime: 0, duration: 0, isPlaying: false, isLoading: false });
			},
			(data) => {
				this.playbackPanelView?.updatePlaybackState(data);
			}
		...
		this.registerInterval(
			window.setInterval(() => {
				this.fileManager.cleanupExpiredTempAudioFile(this.settings.tempAudioRetentionDays);
			}, 60 * 60 * 1000)
		);

		// Add command to read notes aloud
		this.addCommand({
			id: 'open-playback-panel',
			name: '打开语音朗读面板',
			callback: () => {
				this.activatePlaybackPanel();
			}
		});
		...
		this.addCommand({
			...
			editorCallback: (editor, view) => {
				...
				if (selectedText.trim()) {
					...
					if (shouldShowNotices(this.settings)) new Notice('没有选中文本。');
				}
			}
		});
		...
		this.addCommand({
			...
			callback: () => {
				...
				if (shouldShowNotices(this.settings)) new Notice('悬浮控件位置已重置。');
			}
		});
		...
		if (this.settings.enableQueueFeature) {
			this.addCommand({
				...
				name: '将当前笔记加入朗读队列',
				editorCallback: (editor, view) => {
					const noteTitle = view.file?.basename || '未命名';
			...
			this.addCommand({
				...
				editorCallback: (editor, view) => {
					...
					if (selectedText.trim()) {
						const noteTitle = view.file?.basename || '未命名';
						this.audioManager.addToQueue(selectedText, `${noteTitle}（选区）`);
					} else {
						if (shouldShowNotices(this.settings)) new Notice('没有选中文本。');
					}
				}
			...
			this.addCommand({
				...
				callback: () => {
					...
					const message = status.queue.length === 0
						? '朗读队列为空。'
						: `朗读队列共有 ${status.queue.length} 项。${status.isPlayingFromQueue ? `正在播放第 ${status.currentIndex + 1} 项。` : '当前未从队列播放。'}`;
		...
		if (this.settings.enableQueueFeature && this.queueUIManager) {
			...
			this.addCommand({
				...
				callback: () => {
					...
					if (this.settings.showNotices) new Notice('朗读队列位置已重置。');
				}
			});
		}
	...
	async activatePlaybackPanel(): Promise<void> {
		let leaf = this.app.workspace.getLeavesOfType(VIEW_TYPE_EDGE_TTS_PLAYBACK_PANEL)[0];
		if (!leaf) {
			leaf = this.app.workspace.getRightLeaf(false) || this.app.workspace.getLeaf(true);
			await leaf.setViewState({ type: VIEW_TYPE_EDGE_TTS_PLAYBACK_PANEL, active: true });
		}
		this.app.workspace.revealLeaf(leaf);
		this.playbackPanelView?.refresh();
	}
	...
	async readFromCursor(editor?: Editor, view?: MarkdownView | MarkdownFileInfo): Promise<void> {
		...
		if (!editor) {
			if (shouldShowNotices(this.settings)) new Notice('当前没有可朗读的编辑器或笔记视图。');
			return;
		}
		...
		if (textFromCursor.trim()) {
			...
			if (shouldShowNotices(this.settings)) new Notice('光标之后没有可朗读文本。');
		}
	}
	...
	async readNoteAloud(editor?: Editor, viewInput?: MarkdownView | MarkdownFileInfo, filePath?: string): Promise<void> {
		...
		if (filePath) {
			...
			if (fileContent) {
				...
				if (this.settings.showNotices) new Notice('朗读失败：无法读取笔记内容。');
				return;
			}
		} else {
	...
	async generateMP3(editor?: Editor, viewInput?: MarkdownView | MarkdownFileInfo, filePath?: string): Promise<void> {
		...
		if (!selectedText.trim()) {
			if (this.settings.showNotices) new Notice('没有选中文本或可朗读内容。');
			return;
		}
	...
	async loadSettings() {
		...
		if (typeof this.settings.tempAudioRetentionDays === 'undefined') {
			this.settings.tempAudioRetentionDays = DEFAULT_SETTINGS.tempAudioRetentionDays;
		}
